from se3_transformer.utils.utils_profiling import *  # load before other local modules
...
from se3_transformer.equivariant_attention.from_se3cnn import utils_steerable
from se3_transformer.equivariant_attention.fibers import Fiber, fiber2head
from se3_transformer.utils.utils_logging import log_gradient_norm
...
class GConvSE3(nn.Module):
    ...
    def udf_u_mul_e(self, d_out):
        ...
        def fnc(edges):
            ...
            return {'m': msg.view(msg.shape[0], -1, 2*d_out+1)}
        return fnc
    ...
    def forward(self, h, G=None, r=None, basis=None, **kwargs):
        ...
        with G.local_scope():
            ...
            for d in self.f_out.degrees:
                G.update_all(self.udf_u_mul_e(d), fn.mean('m', f'out{d}'))
...
class GMABSE3(nn.Module):
    ...
    def forward(self, v, k: Dict=None, q: Dict=None, G=None, **kwargs):
        ...
        with G.local_scope():
            ...
            # print(edge_softmax(G, e).squeeze().detach().cpu().numpy())
...
class GSE3Res(nn.Module):
    ...
    def forward(self, features, G, **kwargs):
        ...
        # Attention
        z, attn = self.GMAB['attn'](v, k=k, q=q, G=G)
